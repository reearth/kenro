...
| [`ST_QuantizeCoordinates(geom, prec)`](https://postgis.net/docs/ST_QuantizeCoordinates.html) / `(geom, prec_x, prec_y)` / `(…, prec_z)` | geometry | ✅ | ❌ | ❌ | Zeroes the coordinate bits `prec` decimal places cannot distinguish, so the geometry compresses better. **Not a grid** — the result is the input minus bits, not a round decimal; use `ST_SnapToGrid` or `ST_ReducePrecision` for that. PostGIS's bit rule is reproduced **bit-exactly** (2216 golden vectors, `prec` −30…40 over subnormals, ±0 and `f64::MAX`). PostGIS spells the fallbacks as SQL defaults (`prec_y`/`prec_z` NULL → `prec_x`, *not* the argument before); SQLite has no such syntax, so they are arities. ⚠️ **no `prec_m` arity, and a geometry carrying M is an error** — PostGIS quantizes M at `prec_x` and kenro's coordinate walker has no M slot, so raising beats returning an M the name promised to quantize |
